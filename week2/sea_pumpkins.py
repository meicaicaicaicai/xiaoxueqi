...
import matplotlib.pyplot as plt  # seaborn基于matplotlib，仍需导入但使用方式改变
...
variety_prices = variety_prices.groupby('Variety')['Price_per_cuft'].median().sort_values(ascending=False).reset_index()
...
ax = sns.barplot(
    ...
    data=variety_prices,
    color='darkorange',
    order=variety_prices['Variety']  # 保持排序顺序
)
...
# 添加数值标签
for i, v in enumerate(variety_prices['Price_per_cuft']):
...
# 计算价格中位数
size_prices = df_cleaned.groupby('Item Size')['Price_per_cuft'].median().reindex(size_order).reset_index()
...
# 添加数值标签
for i, v in enumerate(size_prices['Price_per_cuft']):
...
# 3. 时间序列分析（月度）
plt.figure(figsize=(10, 6))
monthly_prices = df_cleaned.groupby('Month')['Price_per_cuft'].mean().reset_index()
...
decomposition.trend.plot(color='brown', linewidth=2)
plt.title('南瓜价格长期趋势')
plt.ylabel('价格 ($/cuft)')
plt.grid(True)
plt.tight_layout()
...
local_prices = df_cleaned.groupby('Is_Local')['Price_per_cuft'].mean().reset_index()
local_prices['Is_Local'] = local_prices['Is_Local'].map({0: '外地', 1: '本地'})
...
# 6. 包装类型价格分布
plt.figure(figsize=(10, 6))
package_prices = df_cleaned.groupby('Package')['Price_per_cuft'].median().nlargest(5).reset_index()
